...
Select Player_Name, Length(CONCAT('Mr. ', Player_Name)) "Lenght of Name with Mr", CONCAT(round(g.diving,-1),'%') "Diving Rounded Nearest 10th" FROM player p , goalkeeper g WHERE
...
Select p.Player_Name, FLOOR(MONTHS_BETWEEN(SYSDATE,p.DOB)/12) Age, TO_CHAR(DECODE(TEAM_NAME, 'ManU', 1,
                                                                                        'Barca', 3,
                                                                                        'ManC', 4,
                                                                                        'Aston', 2,
                                                                                        0)) "Team Rank"
...
Select c.coach_name, t.team_name, (TO_NUMBER(TO_CHAR(SYSDATE,'YYYY')) - TO_NUMBER(TO_CHAR(c.join_date,'YYYY'))) "Coaching in Years", TO_CHAR(ADD_MONTHS(c.join_date, 24),'MONTH-YYYY') "Contract Renewal Date" 
FROM team t, coach c WHERE
t.coach_id=c.coach_id;

--Jiyad ur Rehman
--4

Select p.Player_Name, Replace(UPPER(p.Player_position), 'GK', 'Goal Keeper') "Postion", lower(SUBSTR(t.home_ground,0,3)) "Ground Abbreviation"
From Player p, Team t WHERE
p.team_id=t.team_id; 
...
SELECT Match_ID, Max(Goal_Scored) "Maximum Goal Scored in Each Match" FROM TEAM_MATCH
GROUP BY MATCH_ID;
...
WHERE match_id = 3001
GROUP BY MATCH_ID
HAVING Max(Goal_Scored) > 0;

--Jiyad ur Rehman
--8

Select t.Team_Name, c.Coach_Name, Round(Avg(p.Height)) "Average Team Height" FROM player p, team t, coach c
WHERE
...
Select t.Team_Name, p.player_type, Round(Avg(p.Height)) "Average Height" FROM player p, team t
WHERE
p.team_id=t.team_id
GROUP BY ROLLUP  (t.team_name, p.player_type);
...
WHERE
p.team_id=t.team_id
GROUP BY CUBE  (t.team_name, p.player_type);
