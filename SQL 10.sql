...
Select p.player_name, t.team_name FROM
Player p JOIN Team t USING (team_id);
...
Select p.player_name, t.team_name, p.height FROM
Player p, Team t WHERE
t.team_id = p.team_id AND
p.height > 180 AND t.team_name LIKE ('Ma%');

-- JIYAD UR REHMAN
-- 5

Select p.player_name, t.team_name FROM
Player p JOIN Team t USING (team_id) WHERE
p.DOB < '31-DEC-90' AND t.team_name IN ('Aston','Barca');

-- JIYAD UR REHMAN
-- 6

Select p.player_name, t.team_name, c.coach_name FROM
Player p, Team t, coach c WHERE
t.team_id = p.team_id AND
c.coach_id = t.coach_id;

-- JIYAD UR REHMAN
-- 7

Select p.player_name, t.team_name, c.coach_name FROM
Player p JOIN Team t USING (team_id) JOIN coach c USING (coach_id);
...
Select p.player_name, t.team_name, c.coach_name FROM
Player p JOIN Team t ON
t.team_id = p.team_id JOIN coach c ON
c.coach_id = t.coach_id;

-- JIYAD UR REHMAN
-- 9

Select p.player_name, t.team_name, p.height, c.coach_name FROM
Player p, Team t, coach c WHERE
t.team_id = p.team_id AND
c.coach_id = t.coach_id AND
p.height > 180 AND t.team_name LIKE ('Ma%');
...
Select p.player_name, t.team_name, c.coach_name, g.diving FROM
Player p, Team t, coach c, goalkeeper g WHERE
t.team_id = p.team_id AND
c.coach_id = t.coach_id AND
g.player_id = p.player_id AND
t.team_name LIKE ('Ma%') AND
g.diving > 95;
